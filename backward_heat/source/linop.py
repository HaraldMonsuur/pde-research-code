...
def KronLinOp(mat_time, mat_space):
    r""" Apply x \mapsto (A \kron B) x efficiently in a matrix-free way. """
    N, K = mat_time.shape
    M, L = mat_space.shape
...
def BlockDiagLinOp(linops):
    r""" Block diagonal as a linear operator. """
...
def BlockLinOp(linops):
    r""" Block of linear operators as a linear operator. """
...
class CompositeLinOp(sp.linalg.LinearOperator):
    r""" x \mapsto ABx as a linear operator. """
